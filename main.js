...
function broadcast_selection(ev, plot) {
    if(plot.param == 'x') {
        "update selections set value = {ev.x} where plot = {plot} and param = 'x' ";
    }
    if (plot.param == 'y') {
        "update selections set value = {ev.y} where plot = {plot} and param = 'y' ";
    }
}
